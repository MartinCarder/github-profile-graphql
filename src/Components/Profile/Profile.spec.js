...
describe('<Profile />', () => {
  ...
  it('renders correct details', () => {
    ...
    expect(
      wrapper.find('[data-id="name"]').text()
    ).toEqual(data.name)

    expect(
      wrapper.find('[data-id="login"]').text()
    ).toEqual(data.login)

    expect(
      wrapper.find('[data-id="location"]').text()
    ).toEqual(data.location)

    expect(
      wrapper.find('[data-id="company"]').text()
    ).toEqual(data.company)

    expect(
      wrapper.find('[data-id="bio"]').text()
    ).toEqual(data.bio)
  })
